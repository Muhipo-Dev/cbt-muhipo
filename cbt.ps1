...
function Write-Banner {
    ...
    Write-Host "  |    Port Server: :$CBT_PORT (Web) & :$STUDIO_PORT (Prisma Studio)|" -ForegroundColor DarkCyan
    Write-Host "  +==================================================+" -ForegroundColor Cyan
    Write-Host ""
}
...
function Show-Status {
    ...
    $aStatus = if ($appRunning)    { "AKTIF".PadRight(12) } else { "MATI/OFF".PadRight(12) }
    $pStatus = if ($prismaRunning) { "AKTIF".PadRight(12) } else { "MATI/OFF".PadRight(12) }
    ...
    Write-Host "  | CBT Web App (:3010)| " -NoNewline
...
while ($true) {
    ...
    Write-Host "  [3] Jalankan CBT Web App Saja (:3010)" -ForegroundColor Cyan
    ...
    switch ($choice) {
        ...
        "5" { Start-Process "http://localhost:3010" }
        "6" {
