...
const WAITING_TIPS = [
  ...
  },
  {
    icon: "⚡",
    title: "Tahukah Anda?",
    desc: "CBT MUHIPO dilengkapi sistem proteksi otomatis yang menjaga konsistensi pengerjaan ujian dan data jawaban Anda tetap aman 100%.",
  },
  {
    icon: "💡",
    title: "Tips Ujian Tenang",
    desc: "Tarik napas sejenak, baca basmalah, dan siapkan konsentrasi. Giliran ujian Anda akan segera siap dalam beberapa detik!",
  },
  {
    icon: "🌟",
...
const TRIVIA_QUESTIONS = [
  ...
  {
    q: "Surah dalam Al-Qur'an yang menjadi landasan teologi gerakan kepedulian Al-Ma'un adalah?",
    options: ["QS. Al-Kautsar", "QS. Al-Ikhlas", "QS. Al-Ma'un", "QS. Al-Fatihah"],
    answer: 2,
  },
  {
    q: "Apa semboyan utama perjuangan pelajar dan Ikatan Pelajar Muhammadiyah?",
    options: ["Nun Wal Qalami Wa Maa Yasthuruun", "Bhinneka Tunggal Ika", "Tut Wuri Handayani", "Ing Ngarso Sung Tulodo"],
    answer: 0,
  },
  {
    q: "Apa visi utama SMA Muhammadiyah 1 Ponorogo (MUHIPO)?",
    options: ["Islami, Berprestasi, dan Berkarakter", "Modern dan Bebas", "Cepat Lulus", "Kompetitif Semata"],
    answer: 0,
  }
];
...
export default function WaitingRoomProvider({ children }: { children: React.ReactNode }) {
  ...
  // Ganti tips edukasi secara berkala
  useEffect(() => {
  ...
  // Polling Status Waiting Room
  useEffect(() => {
    let token = "";
    try {
      token = localStorage.getItem("cbt_wr_token") || "";
    } catch {
      // ignore
    }
    ...
    const checkStatus = async () => {
      try {
        const res = await fetch(`/api/waiting-room/status?token=${encodeURIComponent(token)}`);
        
        if (res.ok) {
          const data: QueueStatus = await res.json();
          if (data.token) {
            try {
              localStorage.setItem("cbt_wr_token", data.token);
              document.cookie = `cbt_wr_token=${data.token}; path=/; max-age=86400; SameSite=Lax`;
            } catch {
              // ignore
            }
            token = data.token;
        ...
        // Fallback default admitted jika terjadi kendala jaringan lokal
        setQueueState({
          token,
          status: "ADMITTED",
          position: 0,
  ...
  // Global Fetch Interceptor untuk menangkap status 429 Waiting Room
  useEffect(() => {
  ...
  // Jika sedang antre di Waiting Room (status === 'QUEUED')
        ...
        <div className="relative w-full max-w-lg overflow-hidden rounded-2xl sm:rounded-3xl border border-blue-500/30 bg-gradient-to-b from-slate-900/95 via-slate-900/90 to-slate-950/95 p-4 sm:p-6 md:p-8 shadow-2xl shadow-blue-950/60 backdrop-blur-xl transition-all duration-300">
          ...
          <div className="text-center mb-4 sm:mb-5">
            ...
            <p className="text-xs sm:text-sm text-slate-300 max-w-sm mx-auto leading-relaxed">
              Lalu lintas server CBT sedang sangat padat. Anda berada di antrean prioritas untuk menjamin integritas data jawaban ujian Anda.
            </p>
          </div>
          ...
          <div className="space-y-1.5 mb-4 sm:mb-5">
            <div className="flex justify-between items-center text-[11px] sm:text-xs text-slate-400">
              <span className="flex items-center gap-1.5 font-medium text-blue-400">
                <Activity className="w-3.5 h-3.5 animate-spin" /> Memproses Giliran Ujian...
              </span>
              <span className="font-mono text-blue-300 font-semibold">{Math.round(progressPercent)}%</span>
            </div>
            
            <div className="h-2.5 sm:h-3 w-full bg-slate-800/80 rounded-full overflow-hidden p-0.5 border border-slate-700/50">
              <div
                className="h-full bg-gradient-to-r from-blue-500 via-indigo-400 to-amber-300 rounded-full transition-all duration-700 ease-out shadow-[0_0_12px_rgba(59,130,246,0.8)]"
                style={{ width: `${progressPercent}%` }}
              />
            </div>
          </div>

          {/* Interactive Anti-Boredom Tabbed Card */}
          <div className="rounded-2xl bg-slate-800/50 border border-slate-700/60 p-3 sm:p-4 mb-4">
              ...
              <div className="text-left">
                ...
                {selectedAnswer !== null && (
                  <div className="flex justify-between items-center pt-1">
                    <span className={`text-[11px] font-semibold ${isCorrect ? "text-blue-400" : "text-rose-400"}`}>
            ...
            {activeTab === "clicker" && (
              <div className="flex items-center justify-between gap-3 bg-slate-900/60 rounded-xl p-3 border border-slate-700/50">
                <div className="text-left">
                  <div className="text-[11px] text-slate-400">Sentuh untuk meredakan ketegangan sebelum ujian:</div>
                ...
                <button
                  ...
                  className="px-4 py-2 bg-gradient-to-r from-blue-500 via-indigo-500 to-indigo-400 hover:from-blue-400 hover:to-indigo-300 active:scale-95 text-white font-black rounded-xl text-xs transition shadow-lg shadow-blue-950/50 cursor-pointer flex items-center gap-1.5"
                >
          ...
          <div className="text-[10px] sm:text-[11px] text-slate-400 text-center border-t border-slate-800/80 pt-3">
            🔒 Halaman ini akan otomatis beralih begitu giliran Anda tiba. Mohon jangan menutup browser.
          </div>
        </div>
      </div>
    );
